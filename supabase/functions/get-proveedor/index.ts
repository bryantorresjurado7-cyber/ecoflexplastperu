...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[get-proveedor] Parámetros recibidos:', { tipoDocumento, numeroDocumento })
    ...
    if (!tipoDocumento || !numeroDocumento) {
      return new Response(
        JSON.stringify({ 
          ...
          ejemplo: '/get-proveedor?tipo_documento=RUC&numero_documento=12345678901',
          recibidos: { tipoDocumento, numeroDocumento }
        }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    console.log('[get-proveedor] Valores para búsqueda:', { tipoDocumento, numeroDocumento: numeroLimpio })
    ...
    console.log('[get-proveedor] Resultado primera búsqueda:', { data: data ? 'encontrado' : 'no encontrado', error })

    // Si no encontramos, intentar solo con número de documento (puede ser problema de formato)
    if (!data && (!error || error.code === 'PGRST116')) {
      console.log('[get-proveedor] Intentando búsqueda solo por número...')
      ...
      console.log('[get-proveedor] Resultado búsqueda simple:', { data: dataSimple ? 'encontrado' : 'no encontrado', error: errorSimple })
    ...
    if (!data) {
      return new Response(
        JSON.stringify({ 
          ...
          numero_documento: numeroLimpio || null
        }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    console.error('Error en get-proveedor:', err)
